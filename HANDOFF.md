...
A cloud bounded-first-pass run (24 agents, one per shard, capped at ~10 batches each,
no auto-retry of failures) took every shard from 0/33 to **10/33 batches (250/817ish
entries)**, each independently confirmed `qa_shard.py --partial` PASS. Aggregate check
across all 6,000 collected rows: 226/227 slugs used, top slug 9.9% share (cap 15%), 69
distinct confidence values, top mechanism 26.4% share (cap 50%), unclassified 1.4%.

| shard | batches done | gate |
|---|---|---|
| 00-23 (all) | 10/33 | **PASS** (partial — see above) |

Next batch to resume for every shard is `b010`. Run `python3 scripts/qa_shard.py NN`
(full, not `--partial`) once a shard reaches 33/33 before trusting it complete.
